...
export default async function Header({
  ...
  const currentTranslation = translation?.find((item) => item?.lang?.toLowerCase() === locale.toLowerCase())
  return (
    <HeaderAnimation locale={locale} border={border} title={title}>
      <div dir={locale === "ar" ? "rtl" : "ltr"} className="  h-full w-full flex items-center justify-between  sm:px-8 md:px-16 lg:px-24 2xl:px-32 sm:py-4 py-0 px-4 ">
          ...
          <Link href="/" className="flex items-center gap-2 w-max">
            ...
            {companyInfo?.logo ? (
              <Image
                ...
                width={8}
                height={8}
                alt={companyInfo.logo.alt || currentTranslation?.name || ""}
                className="h-8 w-8 object-contain"
              />
            ) : (
              <div className="h-8 w-8 rounded-lg bg-primary flex items-center justify-center text-primary-foreground font-bold">
                {currentTranslation?.name?.charAt(0) ?? "V"}
              </div>
            )}
            <span className="font-semibold ">
              {currentTranslation?.name ?? dictionary.header.brandFallback}
            </span>
          </Link>
        </div>
        ...
        <HeaderClient locale={locale} dictionary={dictionary} companyInfo={{ ...company, ...currentTranslation, logo: logo || undefined }} navItems={navItems} />
      </div>
    </HeaderAnimation>
  );
}

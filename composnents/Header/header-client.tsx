...
export default function HeaderClient({ locale, dictionary, navItems, companyInfo }: {
  dictionary: Awaited<ReturnType<typeof getDictionary>>, navItems: NavItem[], companyInfo: CompanyInfo | null,
  locale: "en" | "ar"
}) {
  ...
  return (
    <Fragment>
      <div className=" p-1  flex-1 text-end  hidden md:flex  items-center  gap-3 justify-end" >
        ...
        <SwitchLang />
        <div className="border border-primary   rounded-md flex items-center justify-center p-[4px] bg-transparent">


          <ThemeToggle placement="header" showIcon showTransition variant="default" />
        </div>
      </div>

      <div className=" md:hidden  flex items-center  justify-center gap-2">
        <button onClick={() => setOpen(true)} className="max-h-max max-w-max h-max w-max min-w-max minh-max cursor-pointer bg-muted/50 hover:bg-muted   p-2 rounded-md">
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed inset-0 bg-black/60 backdrop-blur-sm z-100 opacity-0"
      ...
      <div
        ...
        ref={containerRef}
        className={cn("fixed   overflow-hidden top-0  bg-background border-l border-border z-200 transform translate-x-full",
      ...
      >
        <div ref={contentRef} className="h-full flex flex-col p-6 md:p-8 lg:p-12">
          ...
          {/* Footer */}
          <div className="pt-6 md:pt-8 border-t border-border w-full">
            <button
            onClick={()=>{
              onOpenChange(false)
              router.push("#contact")
            }}
            className=" cursor-pointer flex items-center justify-center w-full py-3 md:py-4 bg-primary text-primary-foreground rounded-full font-medium hover:opacity-90 transition-all duration-300 hover:scale-101">
             {dictionary.header.contact}
            </button >
            ...
            <div className="flex items-center justify-center gap-4 md:gap-6 mt-6 md:mt-8 text-sm text-muted-foreground">
              {
                companyInfo?.twitter &&
                <Link href={companyInfo.twitter} target="_blank" className="hover:text-foreground transition-colors">Twitter</Link>
              }
              {
                companyInfo?.instagram &&
                <Link href={companyInfo?.instagram} target="_blank" className="hover:text-foreground transition-colors">Instagram</Link>
              }
              {
                companyInfo?.linkedin &&
                <Link href={companyInfo?.linkedin} target="_blank" className="hover:text-foreground transition-colors">LinkedIn</Link>
              }
